...
import { Server, Play, Tv, Video, MonitorPlay, Building2, Smartphone, Globe, ArrowRight, Check, Zap, TrendingUp, Shield, Clock, BarChart3 } from "lucide-react";
            ...
            <div className="mb-16 overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)]">
              ...
              >
                {[...["Blasto", "MGID", "Silverpush", "Bidscube", "Bid9", "Kueez", "Smile Wanted", "LoopMe", "Nexverse", "JioAds"], ...["Blasto", "MGID", "Silverpush", "Bidscube", "Bid9", "Kueez", "Smile Wanted", "LoopMe", "Nexverse", "JioAds"]].map((name, i) => (
                  <div
                    key={`row2-${i}`}
                    className="h-16 w-44 flex-shrink-0 rounded-lg bg-card/80 backdrop-blur-sm border border-border/50 flex items-center justify-center text-foreground font-bold text-sm tracking-tight hover:border-highlight/50 hover:bg-highlight/5 hover:scale-105 transition-all duration-300 cursor-default" style={{ fontFamily: "'DM Sans', sans-serif" }}
                  >
                    {name}
                  </div>
                ))}
