...
const Header = () => {
  ...
  return (
    <motion.header
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${isScrolled ? "py-0.5" : "py-1"}`}
    >
      {/* Floating header container */}
      <div className="container mx-auto px-6">
        <div
          className={`flex items-center justify-between transition-all duration-500 ${
            isScrolled ? "bg-card/90 backdrop-blur-xl rounded-2xl px-6 py-1 shadow-lg border border-border/50" : ""
          }`}
        >
          {/* Logo - Extra large */}
          <a
          ...
          >
            ...
            />
            <span
              className={`font-semibold text-muted-foreground transition-all duration-500 ${isScrolled ? "text-[10px] -ml-7 -mt-2" : "text-[12px] -ml-8 -mt-4"}`}
            >
              ™
            </span>
          </a>
